...
const STATUS_TABS = ['ALL', 'PENDING', 'ASSIGNED', 'IN_PROGRESS', 'RESOLVED', 'SLA_BREACH'];
...
export default function OfficerComplaints() {
  ...
  useEffect(() => { fetchComplaints(); }, [activeTab, search, page]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {totalCount} complaints · {user?.department?.name || 'Department'} Queue
          </p>
        </div>
        <button onClick={fetchComplaints} className="p-2 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 transition-colors">
      ...
      <div className="relative max-w-sm">
        ...
        <input
          type="text"
          placeholder="Search reference, title, or address..."
          value={search}
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 border-b border-slate-100">
              <tr>
                ...
                <th className="px-6 py-3 text-xs font-semibold text-slate-500 uppercase">Citizen</th>
                <th className="px-6 py-3 text-xs font-semibold text-slate-500 uppercase">Title / Address</th>
                <th className="px-6 py-3 text-xs font-semibold text-slate-500 uppercase">Priority</th>
                <th className="px-6 py-3 text-xs font-semibold text-slate-500 uppercase">Status</th>
                <th className="px-6 py-3 text-xs font-semibold text-slate-500 uppercase">SLA</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                [...Array(5)].map((_, i) => (
                  <tr key={i}><td colSpan="8" className="px-6 py-4"><div className="h-4 bg-slate-100 rounded animate-pulse" /></td></tr>
                ))
              ) : complaints.length === 0 ? (
                <tr>
                ...
                complaints.map(c => (
                  <tr key={c.id} className={`hover:bg-slate-50 transition-colors ${c.sla_breached ? 'bg-red-50/30' : ''}`}>
                    <td className="px-6 py-3 font-mono text-xs text-slate-500">#{c.reference_number || c.id}</td>
                    <td className="px-6 py-3 font-medium text-slate-700">{c.citizen_name || 'N/A'}</td>
                    <td className="px-6 py-3">
                      <p className="text-slate-800 font-medium truncate max-w-xs">{c.title}</p>
                    ...
                    <td className="px-6 py-3 text-right">
                      <Link to={`/officer/complaints/${c.id}`} className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors">
                        <Eye className="w-3 h-3" /> View & Assign
                      </Link>
                    </td>
                  </tr>
                ))
