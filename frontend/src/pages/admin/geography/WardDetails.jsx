...
import {
  MapPin, Users, Edit, ArrowLeft, AlertTriangle, Shield, Activity, Wifi, Droplets, Zap, Trash2, Truck
} from 'lucide-react';
...
export default function WardDetails() {
  ...
  const fetchWard = async () => {
    try {
      ...
      const [wardRes, statsRes] = await Promise.all([
        api.get(`/geography/wards/${id}/`),
        api.get(`/geography/wards/${id}/stats/`),
      ]);
  ...
  if (!ward) return <div className="p-10 text-center text-slate-500">Geography record not found.</div>;
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex items-center gap-4 text-sm text-slate-500">
        ...
        <span>/</span>
        <span className="text-slate-900 font-medium">Wards</span>
        <span>/</span>
        <span className="text-slate-900 font-medium">Ward {ward.number}</span>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold text-slate-900">Ward {ward.number} {ward.name && `(${ward.name})`}</h1>
            <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${ward.is_active !== false ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
              {ward.is_active !== false ? 'Active' : 'Inactive'}
            </span>
          </div>
          <p className="text-slate-500 mt-1 flex items-center gap-1">
            <MapPin className="w-4 h-4" /> {ward.zone_name ? `${ward.zone_name}, ` : ''}{ward.city_name}
          </p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-sm font-medium transition-colors">
            <Edit className="w-4 h-4" /> Edit Ward
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Traffic Incidents', value: stats?.traffic_incidents, icon: Truck, color: 'text-amber-600', bg: 'bg-amber-50' },
          { label: 'Waste Incidents', value: stats?.waste_incidents, icon: Trash2, color: 'text-stone-600', bg: 'bg-stone-50' },
          { label: 'Water Incidents', value: stats?.water_incidents, icon: Droplets, color: 'text-blue-500', bg: 'bg-blue-50' },
          { label: 'Power Incidents', value: stats?.electricity_incidents, icon: Zap, color: 'text-yellow-600', bg: 'bg-yellow-50' },
        ].map((stat, i) => (
